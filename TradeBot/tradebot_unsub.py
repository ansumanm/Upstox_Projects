...
# Filter example.
def main():
    parser = argparse.ArgumentParser("Instrument unsubscribe")
    parser.add_argument('-s', '--str',help='instrument to unsubscribe')
    parser.add_argument('-c', '--class',help='[MCX_FO, NSE_EQ, BSE_EQ, NSE_FO]')
    ...
    try:
        u.unsubscribe(u.get_instrument_by_symbol(inst_cls, inst_str), LiveFeedType.Full)
        u.unsubscribe(u.get_instrument_by_symbol(inst_cls, inst_str), LiveFeedType.LTP)
    except Exception as e:
