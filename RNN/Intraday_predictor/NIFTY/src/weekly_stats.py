...
# Lets take stats after date from which weekly options was introduced.
...
Open to High
"""
weekly_df['O2H'] = weekly_df['High'] - weekly_df['Open']

# weekly_df['O2C_per'] = (weekly_df['O2C']*100)/weekly_df['Open']
# https://matplotlib.org/3.1.1/api/_as_gen/matplotlib.pyplot.hist.html
# weekly_df['O2C'].hist(bins=10, density=True, cumulative=True).figure.savefig('o2c.png')
# bins=weekly_df['O2H'].quantile([0,0.1,0.20,0.30,0.40,0.5,0.6,0.70,0.80,0.90,1]).to_list()
# weekly_df['O2C'].hist(bins='auto', density=False, cumulative=False, rwidth=0.9).figure.savefig('weekly_o2c.png')
weekly_df['O2H'].hist(bins=50, density=False, cumulative=False, rwidth=0.9).figure.savefig('weekly_o2h.png')
